...
export interface RealMigrationRecordResult {
  step: number
  collection: string
...
export interface StagingMigrationReport {
  ...
  engineVersion: 'v1.0.0-staging-migrator'
  ...
  steps: Array<{
    ...
    updates: number
    errors: number
    records: RealMigrationRecordResult[]
  }>
  journalFile: string
...
export function executeStagingMigration001(): StagingMigrationReport {
  const journalRecords: RealMigrationRecordResult[] = []
  const steps: StagingMigrationReport['steps'] = []
  let totalCreates = 0
  let totalUpdates = 0
  let totalErrors = 0
  ...
  const userResults: RealMigrationRecordResult[] = mockUsers.map((u) => {
    const payload = {
      email: u.email,
      first_name: u.name,
    ...
    const rec: RealMigrationRecordResult = {
      step: 1,
      collection: 'users',
      sourceId: u.id,
      sourceSlug: u.email,
  ...
  const mediaResults: RealMigrationRecordResult[] = mockMedia.map((m) => {
    const payload = {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  const poleResults: RealMigrationRecordResult[] = POLES_SEED_DATA.map((p) => {
    const payload = {
      pole_name_fr: p.name,
      pole_name_en: p.name,
      slug_fr: p.slug,
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  const serviceResults: RealMigrationRecordResult[] = SERVICES_SEED_DATA.map((s) => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  const csResults: RealMigrationRecordResult[] = CASE_STUDIES_SEED_DATA.map((cs) => {
    const payload = {
      title_fr: cs.title,
      title_en: cs.title,
      slug_fr: cs.slug,
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  const postResults: RealMigrationRecordResult[] = POSTS_SEED_DATA.map((p) => {
    const payload = {
      title_fr: p.title,
      title_en: p.title,
      slug_fr: p.slug,
  ...
  const leadResults: RealMigrationRecordResult[] = mockLeads.map((l) => {
    const payload = {
  ...
  const invoiceResults: RealMigrationRecordResult[] = mockInvoices.map((inv) => {
  ...
  const accessReqResults: RealMigrationRecordResult[] = mockAccessReq.map((ar) => {
    const payload = {
  ...
  const globalRec: RealMigrationRecordResult = {
    step: 11,
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // ÉCRITURE DU JOURNAL DE MIGRATION LOCAL
  // ═══════════════════════════════════════════════════════════════════════════
  const journalPath = path.resolve(process.cwd(), 'scripts/migration/staging-migration-001-journal.json')
...
if (process.argv[1]?.includes('orchestrate-staging-migration-001')) {
  const rep = executeStagingMigration001()
  console.log('═══════════════════════════════════════════════════════════════')
  console.log('       RÉSULTAT DE LA MIGRATION CONTRÔLÉE STAGING 001          ')
  console.log('═══════════════════════════════════════════════════════════════')
