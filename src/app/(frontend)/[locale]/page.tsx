...
export default async function HomePage({ params }: PageProps) {
  ...
  const featuredCases = await getCaseStudies(true, currentLocale)
  const itServices = await getServices('it', currentLocale)
          ...
          <div className="container-v4 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              <div className="lg:col-span-5">
                <Kicker>{t.home.itFocusKicker}</Kicker>
                ...
                <Link
                  href={`/${currentLocale}/expertises/it`}
                  className="btn-v4-primary inline-flex items-center gap-2"
                >
                  {t.home.itFocusCta} <span>→</span>
                </Link>
              </div>
