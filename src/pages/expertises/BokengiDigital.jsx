            ...
            {offerings.map((off, idx) => (
              <div key={idx} style={{ padding: '2.5rem', background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-xs)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', textTransform: 'uppercase', color: 'var(--blue-cyan)', fontWeight: 700 }}>Bokengi Digital</span>
                </div>
                <h2 style={{ fontSize: '1.45rem', margin: '0.5rem 0 0.85rem' }}>{off.title}</h2>
                  ...
                  <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                    {off.features.map((feat, fIdx) => (
                      <li key={fIdx} style={{ display: 'flex', gap: '0.6rem', fontSize: '0.92rem', color: 'var(--ink-body)' }}>
                        <span style={{ color: 'var(--blue-cyan)', fontWeight: 700 }}>•</span> {feat}
                      </li>
                    ))}
                  </ul>
                </div>
